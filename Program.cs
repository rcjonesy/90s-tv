...
{
    //transforming the list to only include the name
...
// 4. Return a filtered list of shows whose title contains an & character.
...
{
    return shows.OrderBy(s => s.ImdbRating).Take(shows.Count() - 1).ToList();
}
...
static Show MostEpisodes(List<Show> shows)
{
    ...
    return shows.First(s => s.EpisodeCount == maxEpisodes);
}
...
{
    return shows.Where(s => s.Genres.Contains("Drama")).OrderByDescending(s => s.ImdbRating).Take( shows.Count - 1 ).ToList();
}

// 19. Return the number of crime shows with an IMDB rating greater than 7.0.show
static int GoodCrimeShows(List<Show> shows)
{
    return shows.Where( s => s.ImdbRating > 7.0 ).Count(s => s.Genres.Contains("Crime"));
}
...
{
    return shows.OrderBy( s => s.Name ).FirstOrDefault(s => s.EndYear - s.StartYear > 10 && s.ImdbRating < 8.0);
}
...
static string AllNamesWithCommasPlsAnd(List<Show> shows)
{
    if (shows.Count >= 2)
    {
        //creates string of all but the last word 
        string allButLast = string.Join(", ", shows.Take(shows.Count - 1).Select(s => s.Name));
        string last = shows.Last().Name;
